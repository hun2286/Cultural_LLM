...
- 가능한 경우, 문서 내용을 바탕으로 구체적 사례, 역사적 배경, 의미, 중요성 등을 함께 설명하여 답변을 풍부하게 만드세요.
- 필요하면 내용 이해를 돕는 간단한 설명이나 맥락을 추가할 수 있지만, PDF에 없는 정보는 포함하지 마세요.
...
def title_matches_keywords(title: str, keywords: list):
    ...
    print(f"제목: {title}")
    print(f"키워드 매칭 개수: {matches_count}/{len(keywords)} → 매칭 여부: {is_match}")
    return is_match
