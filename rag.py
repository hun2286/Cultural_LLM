# 스캔본 제외하고 PDF RAG 작업 코드 (문화유산 폴더 전체 임베딩)
# 진행률 표시 포함, 폴더 내부 폴더의 PDF까지 처리
...
# 문화유산 폴더 전체 재귀 탐색 (진행률 표시 포함)
def load_all_pdfs_recursive(root_folder):
    all_docs = []
    failed_pdfs = []

    # 총 PDF 개수
    total_files = sum(len([f for f in files if f.lower().endswith(".pdf")]) for _, _, files in os.walk(root_folder))
    processed_count = 0
        ...
        for pdf_file in pdf_files:
            ...
            try:
                ...
                if not docs:
                    ...
                    print(f"[{processed_count}/{total_files}] {pdf_path} → 텍스트 부족 / 청크 생성 불가")
                else:
                    all_docs.extend(docs)
                    print(f"[{processed_count}/{total_files}] {pdf_path} 처리 완료 ({len(docs)} 문서)")
            except Exception as e:
                print(f"[{processed_count}/{total_files}] {pdf_path} 처리 중 오류: {e}")
                failed_pdfs.append(pdf_path)
    ...
    if docs:
        ...
        for idx, doc in enumerate(docs, 1):
            ...
            if idx % 10 == 0 or idx == total_docs:
                print(f"청크 진행: {idx}/{total_docs} 문서 완료, 총 청크 수: {len(split_docs)}")

        print(f"\n총 청크 수: {len(split_docs)}")
